...
async function createHistoricalReports() {
  try {
    ...
    if (academicYears.length === 0) {
      console.log('❌ No academic years found. Please run academic year seed first.');
      return;
    }
    ...
    if (reporters.length === 0) {
      console.log('❌ No reporters found. Please ensure teachers/BK users exist.');
      return;
    }

    console.log(`Found ${academicYears.length} academic years, ${students.length} students, ${violations.length} violations, ${achievements.length} achievements`);

    // Create historical reports for each inactive academic year
    for (const academicYear of academicYears) {
      if (academicYear.isActive) {
        console.log(`⏭️  Skipping active academic year: ${academicYear.tahunAjaran}`);
        continue;
      }
      ...
      console.log(`✅ Created ${reports.length} reports for ${academicYear.tahunAjaran}`);
    }
    ...
    if (violations.length === 0) {
      ...
      const sampleViolations = [
        { nama: 'Terlambat', kategori: 'ringan', jenis: 'kedisiplinan', point: 5 },
        { nama: 'Tidak mengerjakan tugas', kategori: 'ringan', jenis: 'akademik', point: 10 },
        { nama: 'Bolos tanpa keterangan', kategori: 'sedang', jenis: 'kedisiplinan', point: 15 },
        { nama: 'Berkelahi', kategori: 'berat', jenis: 'kedisiplinan', point: 50 },
        { nama: 'Menyontek saat ujian', kategori: 'berat', jenis: 'akademik', point: 40 }
      ];
    ...
    if (achievements.length === 0) {
      ...
      const sampleAchievements = [
        { nama: 'Juara 1 Olimpiade Matematika', kategori: 'akademik', point: 100 },
    ...
    for (const academicYear of academicYears) {
      ...
      console.log(`- ${academicYear.tahunAjaran}: ${yearReports} reports (${violationCount} violations, ${achievementCount} achievements)`);
    }
